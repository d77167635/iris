...
function readIrisPage() {
  ...
  if (hash.startsWith("workspace/")) return hash.slice("workspace/".length) || "iris";
  return "iris";
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (callback === "confirmed" && data.session) { clearAuthCallback(); setIrisPage("iris"); }
      setCheckedAuth(true);
    ...
    return () => { active = false; listener.subscription.unsubscribe(); window.removeEventListener("hashchange", sync); window.removeEventListener("popstate", sync); };
  }, []);

  const navigate = (page = "iris") => {
    const canonical = canonicalPage(page);
    const hash = canonical === "iris" ? "#workspace/iris" : `#workspace/${canonical}`;
    if (window.location.hash !== hash) window.location.hash = hash.slice(1); else setIrisPage(canonical);
  };
  const signOut = async () => {
