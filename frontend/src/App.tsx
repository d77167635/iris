...
export default function App() {
  ...
  useEffect(() => { let active = true; const recoveryUrl = isRecoveryUrl(); const callback = authCallbackKind(); supabase.auth.getSession().then(({ data }) => { if (!active) return; setSession(data.session); if (recoveryUrl && data.session) setRecovery(true); if (callback === "confirmed" && data.session) clearAuthCallback(); setCheckedAuth(true); }).catch(() => { if (active) { setSession(null); setCheckedAuth(true); } }); const { data: listener } = supabase.auth.onAuthStateChange((event, newSession) => { if (!active) return; if (event === "PASSWORD_RECOVERY") setRecovery(true); setSession(newSession); if (event === "SIGNED_IN" && authCallbackKind() === "confirmed") clearAuthCallback(); }); return () => { active = false; listener.subscription.unsubscribe(); }; }, []);
