...
import "./IrisHierarchyLevel3.css";
    ...
    <div className="ih-layout"><aside className="ih-nav"><div className="ih-nav-title">Your financial life</div><button className={active === "overview" ? "active" : ""} onClick={() => setActive("overview")}>Overview</button>{domains.map(d => <button key={d.domain_key} className={active === d.domain_key ? "active" : ""} onClick={() => setActive(d.domain_key)}>{names[d.domain_key] ?? pretty(d.domain_key)}<State state={d.evidence_state}/></button>)}</aside>
      <section className="ih-content">{active === "overview" ? <>
        ...
        <section className="ih-section"><div className="ih-section-head"><div><span className="ih-eyebrow">Level 3+ · Recursive intelligence</span><h2>Intelligence graph</h2></div><span>{level3Running ? "Building from certified Level 2" : level3?.certified ? "Certified · Published" : "Insufficient evidence"}</span></div>
          {level3?.certified ? <>
            <div className="ih-level3-grid"><article className="ih-level3-card"><span>Recursive capabilities</span><State state="CERTIFIED"/><strong>{level3.capability_count ?? level3.capabilities?.length ?? 0}</strong><small>Governed operators resolved through dependency order</small></article><article className="ih-level3-card"><span>Published intelligence nodes</span><State state="CERTIFIED"/><strong>{level3.materialized?.nodes ?? 0}</strong><small>Materialized only after execution, validation and certification</small></article><article className="ih-level3-card"><span>Parent hierarchy</span><State state="CERTIFIED"/><strong>Level 2</strong><small>Bound to the certified published Level 2 execution</small></article><article className="ih-level3-card"><span>Recursive depth</span><State state="CALCULATED"/><strong>{Math.max(0, ...(level3.capabilities ?? []).map(c => typeof c.recursive_depth === "number" ? c.recursive_depth : 0))}</strong><small>Persisted ancestry depth; no semantic depth ceiling</small></article></div>
