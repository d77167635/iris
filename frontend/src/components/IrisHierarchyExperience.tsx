...
function TransactionRecord({ field }: { field: Field }) {
  const value = (field.value ?? {}) as Record<string, unknown>;
  const amount = typeof value.amount === "number" ? value.amount : null;
  const date = typeof value.posted_date === "string" ? new Date(`${value.posted_date}T00:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "Date not observed";
  const classification = typeof value.transaction_class === "string" ? value.transaction_class : "unknown";
