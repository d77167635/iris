...
export function IrisRawPlaidSupabaseSurface() {
  ...
  useEffect(() => { let active = true; const load = async () => { try { let value: any; try { value = await api.get("/iris/level1"); } catch { await api.post?.("/iris/level1/run"); value = await api.get("/iris/level1"); } if (active) setData(value as Level1); } catch (err: any) { if (active) setError(err?.message ?? "Level 1 is not available"); } }; load(); return () => { active = false; }; }, []);
