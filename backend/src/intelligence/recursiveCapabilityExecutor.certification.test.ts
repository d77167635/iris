...
test("recursive execution never materializes hierarchy nodes before certification", async () => {
  const plan: CapabilityPlan = {
    ...
    resource_estimate: { nodes: 1, edges: 0, compositions: 0 },
    status: "READY",
    limitations: [],
  };
  ...
  const result = await executeRecursiveCapabilityPlan(
    ...
    plan,
    {
      persistGraphNode: async () => {
        hierarchyWriteAttempted = true;
        throw new Error("PRE_CERTIFICATION_HIERARCHY_WRITE");
      },
    },
    { maxNodes: 10, maxEdges: 10, maxCompositions: 10 },
    async ({ userId, capabilityId }) => ({
