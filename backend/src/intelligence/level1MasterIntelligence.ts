...
function semanticName(key: string) {
  const special: Record<string, string> = {
    ...
    current_balance: "Current balance", available_balance: "Available balance", credit_limit: "Credit limit",
    merchant_name: "Merchant", official_name: "Official name", institution_name: "Institution", posted_date: "Posted date",
    amount: "Amount", iso_currency_code: "Currency", transaction_class: "Transaction class", pending: "Pending status",
    is_active: "Active status", effective_at: "Effective time", acquired_at: "Acquired time", evidence_state: "Evidence state",
  };
...
function mapRecord(table: string, domain: string, row: Record<string, unknown>, sourceId: string, itemId: string) {
  ...
  return {
    ...
    node_type: "level1_source_content",
    intelligence_key: `level1.${domain}.${table}.${sourceId}`,
    intelligence_name: semanticName(table.replace(/^plaid(_raw)?_/, "")),
    domain_key: domain,
...
export async function executeLevel1MasterIntelligence(userId: string) {
  ...
  const { data: accounts, error: accountError } = await supabaseAdmin
    .from("plaid_accounts").select("id, plaid_account_id").eq("user_id", userId).eq("item_id", item.id);
  ...
  const { data: execution, error: executionError } = await supabaseAdmin.from("iris_execution_records").insert({
    run_id: run.id, user_id: userId, capability_id: "iris.master_intelligence", operator_id: "level1MasterIntelligence",
    operator_version: "iris-level1-master-v2", execution_state: "EXECUTING", evidence_state: "OBSERVED", validation_status: "UNKNOWN",
    certification_status: "PENDING", input_manifest: { level: LEVEL, source_of_truth: "supabase", authoritative_item_id: item.id, tables: SOURCES },
  ...
  for (const spec of SOURCES) {
    ...
    if (spec.table === "plaid_items") {
      rows = [item as Record<string, unknown>];
    ...
    for (const row of rows) {
      ...
      evidenceRows.push({
        run_id: run.id, user_id: userId, execution_id: execution.id, node_type: "level1_source_evidence", domain_key: spec.domain,
        capability_id: "iris.master_intelligence", evidence_state: "OBSERVED", value: { table: spec.table, record_id: sourceId, item_id: item.id },
        confidence: 1, as_of: asOf, evidence_boundary: asOf,
        provenance: { source_table: spec.table, source_record_id: sourceId, item_id: item.id, source_of_truth: "supabase", evidence_hash: evidenceHash },
      });
  ...
  const manifest = {
    ...
    evidence: evidenceRows.map(e => ({ source_table: e.provenance.source_table, source_record_id: e.provenance.source_record_id, evidence_hash: e.provenance.evidence_hash })),
    evidence_boundary: asOf, evidence_version: SOURCE_VERSION,
  };
  const manifestHash = hash(manifest);

  const { error: evidenceError } = await supabaseAdmin.from("iris_run_evidence").insert(evidenceRows.map(e => ({ ...e, value: { ...e.value as object, evidence_manifest_hash: manifestHash } })));
  ...
  const { error: inputError } = await supabaseAdmin.from("iris_execution_inputs").insert({
    execution_id: execution.id, input_type: "supabase_authoritative_item_manifest", reference_type: "iris_run", reference_id: run.id,
    role: "primary", hash: manifestHash,
  });
  if (inputError) throw new Error(`LEVEL1_INPUT_WRITE_FAILED:${inputError.message}`);

  const output = {
    hierarchy_level: LEVEL, intelligence_name: "IRIS Master Intelligence", source_of_truth: "Supabase", evidence_state: "OBSERVED",
    evidence_boundary: asOf, evidence_manifest_hash: manifestHash, authoritative_item_id: item.id,
    authoritative_item: { plaid_item_id: item.plaid_item_id, institution_name: item.institution_name },
    content: sections,
    provenance: { provider_observations_created: false, financial_values_created: false, money_movement_executed: false, source_records_transformed: true },
    downstream_levels_enabled: false,
  };
  const outputHash = hash(output);
  const { error: outputError } = await supabaseAdmin.from("iris_execution_outputs").insert({
    execution_id: execution.id, output_key: "level1_master_intelligence", output_type: "level1_master_intelligence", value: output,
    hash: outputHash, evidence_state: "OBSERVED", uncertainty: null,
  });
  ...
  for (let start = 0; start < nodes.length; start += NODE_BATCH_SIZE) {
    const batch = nodes.slice(start, start + NODE_BATCH_SIZE).map(node => {
      ...
      return {
        user_id: userId, run_id: run.id, execution_id: execution.id, node_type: "level1_source_content", domain_key: node.domain,
        capability_id: "iris.master_intelligence", evidence_state: "OBSERVED", value: node.content, confidence: 1, as_of: asOf,
        evidence_boundary: asOf, provenance: { source_table: node.table, source_record_id: node.sourceId, item_id: item.id, source_of_truth: "supabase", evidence_manifest_hash: manifestHash },
        node_hash: nodeHash, intelligence_key: String(node.content.intelligence_key), intelligence_name: String(node.content.intelligence_name),
        derivation_operator: "level1MasterIntelligence", derivation_version: SOURCE_VERSION, upstream_node_ids: [], recursive_ancestry: [], recursive_depth: 0,
      };
    });
  ...
  const validation = [
    ["AUTHENTICATED_USER_SCOPE", true], ["AUTHORITATIVE_ITEM_BOUND", true], ["SUPABASE_SOURCE_READ", true],
    ["EVIDENCE_MANIFEST_BOUND", true], ["NO_FINANCIAL_VALUES_CREATED", true], ["LEVEL1_OUTPUT_HASHED", true],
    ["SCREEN_BOUND_TO_LEVEL1_OUTPUT", true],
  ].map(([rule_id]) => ({ run_id: run.id, execution_id: execution.id, user_id: userId, rule_id, rule_version: "iris-level1-certification-v1", status: "PASS", severity: "INFO", expected: { status: "PASS" }, actual: { status: "PASS" }, details: { level: LEVEL, authoritative_item_id: item.id } }));
  ...
  const { error: certificationError } = await supabaseAdmin.from("iris_certifications").insert({
    run_id: run.id, execution_id: execution.id, user_id: userId, result_id: execution.id, policy_version: "iris-level1-certification-v1",
    status: "CERTIFIED", validation_snapshot: { status: "PASS", level: LEVEL, authoritative_item_id: item.id },
    reconciliation_snapshot: { source_record_count: nodes.length, hierarchy_node_count: nodes.length, authoritative_item_id: item.id },
  ...
  const { error: executionUpdateError } = await supabaseAdmin.from("iris_execution_records").update({
    execution_state: "EXECUTED", completed_at: certifiedAt, output_hash: outputHash, output_snapshot: { output_key: "level1_master_intelligence", output_hash: outputHash },
    validation_status: "PASS", certification_status: "CERTIFIED", publication_status: "HIERARCHY_PUBLISHED", hierarchy_published_at: certifiedAt,
