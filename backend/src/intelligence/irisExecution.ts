...
export async function executeIrisRun(request: RunRequest) {
  ...
  if (!selectedItemId) throw new Error("CANONICAL_PROVIDER_ITEM_REQUIRED: full intelligence requires one independently governed canonical Plaid Item; user-level Item aggregation is not silently performed.");
  const evidenceScope: IrisEvidenceScope = { kind: "provider_item", selectedItemId, canonicalProviderDomains: IRIS_CANONICAL_PROVIDER_DOMAINS, deferredProviderDomains: ["statements"] };
  ...
  const { data: run, error: runError } = await supabaseAdmin.from("iris_runs").insert({
    ...
    status: "PLANNED", as_of: asOf, evidence_boundary: asOf, evidence_version: "provider-observation-boundary-v4",
    resource_budget: { max_execution_time_ms: 120000, max_graph_nodes: 10000, max_graph_edges: 30000, max_compositions: 5000 },
    execution_policy: { evidence_gated: true, certify_only_after_validation: true, server_authoritative: true, capability_plan_status: plan.status, evidence_scope_kind: evidenceScope.kind, selected_item_id: selectedItemId, canonical_provider_domains: [...IRIS_CANONICAL_PROVIDER_DOMAINS], deferred_provider_domains: ["statements"], recursive_executor: EXECUTOR_OPERATOR_ID, recursive_executor_version: EXECUTOR_OPERATOR_VERSION },
  ...
  try {
    ...
    if (accountError) throw new Error(`RUN_EVIDENCE_ACCOUNT_SCOPE_FAILED: ${accountError.message}`);
    const accountIds = (accounts ?? []).map(row => row.id).filter((id): id is string => typeof id === "string");

    const [productQuery, transactionQuery, balanceQuery, liabilityQuery] = await Promise.all([
      supabaseAdmin.from("plaid_raw_product_observations").select("id,item_id,product,raw_response,effective_at,acquired_at").eq("user_id", userId).eq("item_id", selectedItemId).eq("is_current", true).eq("evidence_state", "observed"),
      accountIds.length ? supabaseAdmin.from("plaid_raw_transactions").select("id,account_id,raw_response,effective_at,acquired_at").eq("user_id", userId).in("account_id", accountIds).eq("is_current", true).eq("evidence_state", "observed") : Promise.resolve({ data: [], error: null }),
      accountIds.length ? supabaseAdmin.from("plaid_raw_balances").select("id,account_id,raw_response,effective_at,acquired_at").eq("user_id", userId).in("account_id", accountIds).eq("is_current", true).eq("evidence_state", "observed") : Promise.resolve({ data: [], error: null }),
      accountIds.length ? supabaseAdmin.from("plaid_raw_liabilities").select("id,account_id,raw_response,effective_at,acquired_at").eq("user_id", userId).in("account_id", accountIds).eq("is_current", true).eq("evidence_state", "observed") : Promise.resolve({ data: [], error: null }),
    ]);
    const queryErrors = [productQuery, transactionQuery, balanceQuery, liabilityQuery].filter(query => query.error).map(query => query.error!.message);
    if (queryErrors.length) throw new Error(`RUN_EVIDENCE_SOURCE_READ_FAILED: ${queryErrors.join("; ")}`);

    const evidenceRows = [
      ...(productQuery.data ?? []).map(e => ({ run_id: run.id, user_id: userId, evidence_type: "provider_raw_observation", provider: "plaid", product: e.product, raw_observation_id: e.id, effective_at: e.effective_at ?? e.acquired_at, acquired_at: e.acquired_at, evidence_hash: hash(e.raw_response) })),
      ...(transactionQuery.data ?? []).map(e => ({ run_id: run.id, user_id: userId, evidence_type: "provider_raw_transaction", provider: "plaid", product: "transactions", raw_observation_id: e.id, effective_at: e.effective_at ?? e.acquired_at, acquired_at: e.acquired_at, evidence_hash: hash(e.raw_response) })),
    ...
    const executionManifest = { ...initialManifest, run_id: run.id, run_evidence_ids: [...runEvidenceIds].sort(), evidence_count: runEvidenceIds.length, evidence_counts: { products: productQuery.data?.length ?? 0, transactions: transactionQuery.data?.length ?? 0, balances: balanceQuery.data?.length ?? 0, liabilities: liabilityQuery.data?.length ?? 0 } };
    ...
    try {
      const recursive = await executeRecursiveCapabilityPlan(userId, plan, { runId: run.id, executionId: execution.id, asOf, evidenceBoundary: run.evidence_boundary, evidenceManifestHash: run.evidence_manifest_hash, runEvidenceIds, persistGraphNode: ({ capabilityId, result, dependencyResults, dependencyNodeIds }) => persistUserIntelligenceGraph({ userId, runId: run.id, executionId: execution.id, capabilityId, result, dependencyResults, dependencyNodeIds }), persistLineage: ({ capabilityId, result, dependencyResults }) => persistRecursiveLineage({ userId, runId: run.id, executionId: execution.id, capabilityId, result, dependencyResults, runEvidenceIds }) }, { maxNodes: 10000, maxEdges: 30000, maxCompositions: 5000 });
      ...
      const higherOrderFindings = emergentResult && typeof emergentResult === "object" && Array.isArray((emergentResult as Record<string, unknown>).higher_order_findings) ? (emergentResult as { higher_order_findings: Parameters<typeof materializeArbitraryRecursiveCompositions>[0]["synthesis"]["higher_order_findings"] }).higher_order_findings : null;
      const arbitraryRecursiveCompositions = higherOrderFindings ? await materializeArbitraryRecursiveCompositions({ userId, runId: run.id, executionId: execution.id, capabilityNodeIds: recursive.graph_node_ids, evidenceBoundary: run.evidence_boundary, evidenceManifestHash: run.evidence_manifest_hash, runEvidenceIds, synthesis: { ...(emergentResult as object), higher_order_findings: higherOrderFindings } as Parameters<typeof materializeArbitraryRecursiveCompositions>[0]["synthesis"] }) : { materializedNodeIds: [], skippedFindingIds: [] };

      const result = { architecture_version: "IRIS_RECURSIVE_CAPABILITY_GRAPH_V2", execution_status: recursive.status, requested_capabilities: requestedCapabilities, ordered_capabilities: recursive.ordered_capabilities, executed_capabilities: recursive.executed_capabilities, results: recursive.results, graph_node_ids: recursive.graph_node_ids, resource_usage: recursive.resource_usage, arbitrary_recursive_compositions: arbitraryRecursiveCompositions, evidence_scope: evidenceScope, evidence_boundary: run.evidence_boundary, provenance: { source: "governed_capability_registry_and_run_bound_evidence", run_id: run.id, run_evidence_ids: [...runEvidenceIds].sort(), evidence_manifest_hash: run.evidence_manifest_hash, planner_version: PLANNER_VERSION, executor_version: EXECUTOR_OPERATOR_VERSION, persisted_graph_version: "iris-persisted-intelligence-graph-v6", arbitrary_composition_version: "iris-arbitrary-recursive-composition-v2", financial_values_created: false, provider_observations_created: false, money_movement_executed: false } };
