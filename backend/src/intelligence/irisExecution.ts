...
export async function executeIrisRun(request: RunRequest) {
  ...
  // Product-observation inserts can expand through the database's authoritative run-evidence trigger into exact
  // transaction/balance/liability evidence. Re-read after the insert so the full-intelligence manifest and lineage
  // bind to the expanded evidence set rather than only the parent product observations.
