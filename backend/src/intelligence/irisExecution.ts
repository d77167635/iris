...
export async function executeIrisRun(request: RunRequest) {
  ...
  try {
    ...
    try { await materializeIrisUserReportInventory({ userId, runId: run.id, executionId: execution.id, certificationHash }); } catch (reportError) { console.error("IRIS user report inventory materialization failed after certification:", reportError); }
