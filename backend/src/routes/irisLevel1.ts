...
irisLevel1Router.get("/iris/level1", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    const { data: output } = await supabaseAdmin.from("iris_execution_outputs").select("value,hash,evidence_state").eq("execution_id", execution?.id ?? "").eq("output_key", "level1_governance_certificate").maybeSingle();
    if (!execution || execution.certification_status !== "CERTIFIED" || execution.validation_status !== "PASS" || !certification || certification.status !== "CERTIFIED" || !certification.certification_hash || !output?.value) {
      return res.status(409).json({ certified: false, level: 1, error: "Level 1 certification/output is incomplete" });
    }
