 ...
 *
 * The hierarchy catalog is structural ontology only: Level 1 IRIS, Level 2
 * authoritative domains, and Level 3 registered capabilities. It contains no
 * user financial observations and is never treated as financial evidence.
 *
 * Publication is server-gated: a persisted execution output is not exposed as
 * user-facing intelligence unless the authoritative Iris run is CERTIFIED and
 * has a matching persisted certification record.
 */
irisIntelligenceRouter.get("/iris/intelligence", requireAuth, async (req: AuthedRequest, res) => {
  try {
    const [{ data: catalogNodes, error: catalogNodeError }, { data: catalogEdges, error: catalogEdgeError }] = await Promise.all([
      supabaseAdmin
        .from("iris_hierarchy_catalog_nodes")
        .select("node_key,level,node_type,label,parent_key,domain_key,capability_id,sort_order,metadata")
        .order("level", { ascending: true })
        .order("sort_order", { ascending: true }),
      supabaseAdmin
        .from("iris_hierarchy_catalog_edges")
        .select("from_node_key,to_node_key,relation")
        .order("from_node_key", { ascending: true })
        .order("to_node_key", { ascending: true }),
    ]);
    if (catalogNodeError) throw catalogNodeError;
    if (catalogEdgeError) throw catalogEdgeError;
    const hierarchy_catalog = { nodes: catalogNodes ?? [], edges: catalogEdges ?? [] };
    ...
    if (!run) return res.status(503).json({ error: "Iris has not executed a governed intelligence run yet", certified: false, run_id: null, status: "NOT_RUN", hierarchy_catalog });
    ...
    if (!execution) return res.status(503).json({ error: "The latest Iris run has no execution record", certified: false, run_id: run.id, status: run.status, hierarchy_catalog });
    ...
    if (!output?.value) return res.status(503).json({ error: "The latest Iris run has no persisted intelligence output", certified: false, run_id: run.id, execution_id: execution.id, status: run.status, failure_code: run.failure_code ?? null, failure_message: run.failure_message ?? null, hierarchy_catalog });

    const certificationHash = certification?.certification_hash ?? null;
    const certified = run.status === "CERTIFIED" && certification?.status === "CERTIFIED" && typeof certificationHash === "string" && certificationHash.length > 0;
    if (!certified) {
